...
For example, the pan events are normally handled by the ```touchNavigate``` plugin, but in some cases the dnUser might want to interact with other elements like cursors or markes which are on top of the plot.
...
Some of the things this plugin is doing in order to extract the gestures or the dnUser intentions:
* doesn't emit some pan or pinch events unless the dnUser moved its finger(s) a minimum distance
...
After determining the element that the dnUser wants to interact with then this plugin will call one of the ```zoom```, ```pan``` or ```recenter``` of the ```navigate``` plugin.
