...
Same goes for Chrome on Android (all versions) with `dnUser-scalable=no` in the viewport meta tag. But be aware that `dnUser-scalable=no` also disables pinch zooming, which may be an accessibility concern.
